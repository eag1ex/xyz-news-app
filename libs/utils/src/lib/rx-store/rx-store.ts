...
export class RxStore<T extends Object> extends Observable<T> {
  ...
  debug(state: T): void {
    ...
    group.call(
      console,
      `${timestamp} [${this.constructor.name}.${action}] ${changed ? '' : notice}`
    );

    if (this.options.prev) {
      console.log(
        `%c ${padStart('prev state:', 15, ' ')}`,
    ...
    if (this.options.current) {
      console.log(
        `%c ${padStart('current state:', 15, ' ')}`,
        `color: ${colors.current}; font-weight: bold`,
        state
      );
    }
